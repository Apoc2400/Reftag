...
class IsbnDoesNotExistError(IsbnError): pass
...
def hyphenate(input):
    ...
    if not isbn_lengthmaps.groups_length.has_key(GS1_prefix):
        raise IsbnDoesNotExistError, "GS1 prefix %s not recognized" % GS1_prefix
    ...
    if groupPrefixLength is None:
        raise IsbnDoesNotExistError, "Not in any recognized group range"
    elif groupPrefixLength == 0:
        raise IsbnDoesNotExistError, "Group range is unused"
    ...
    if not isbn_lengthmaps.publisher_length.has_key(GS1_and_group):
        raise IsbnDoesNotExistError, "Prefix %s not recognized" % GS1_and_group
    ...
    if publisherLength is None:
        raise IsbnDoesNotExistError, "Not in any recognized publisher range"
    elif publisherLength == 0:
        raise IsbnDoesNotExistError, "Publisher range is unused"
